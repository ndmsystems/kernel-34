...
__IMEM static inline void unmask_mips_irq(struct irq_data *d)
{
	unsigned long flags;
...
__IMEM static inline void mask_mips_irq(struct irq_data *d)
{
	unsigned long flags;
	unsigned int irq = d->irq;
	int cpu = smp_processor_id();
...
#ifdef CONFIG_MIPS_MT_SMTC
	setup_irq_smtc(TIMER5_INT, &tc_watchdog_timer_irqaction, 0x0);
#else
...
#ifdef CONFIG_MIPS_MT_SMTC
	setup_irq_smtc(BUS_TOUT_INT, &tc_bus_timeout_irqaction, 0x0);
#else
